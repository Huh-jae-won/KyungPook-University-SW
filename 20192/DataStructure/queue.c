// queue를 구현하는 queueADT.h 헤더파일을 만들고
// queue를 이용하는 예제를 만듦
#include <stdio.h>
#include <stdbool.h>
#include <stdlib.h>
...
int main() {
	...
	while (1) {
		...
		scanf_s("%d", arrNum+cnt);						// 입력한 수를 arrNum배열에 저장
		while (1) {
			if (arrNum[cnt]>0&&(arrNum[cnt] < 1000 || arrNum[cnt]>9999)) {	// 입력한 수가 4자리가 아니라면 새로 입력받도록 함
				printf("[입력오류]4자리 숫자를 다시 입력하세요! : ");
				scanf_s("%d", arrNum + cnt);
			}else
				break;
		}
		if (arrNum[cnt] == 0)	// 입력한 수가 0일경우 바로 입력 종료
			break;
		cnt++;
		if (cnt >= 10)		// 입력한 수가 최대 10개 까지 가능하도록
			break;
	}
	
	// 입력갯수만큼 큐를 만들고 각 큐에 해당 숫자를 한자리씩 enqueue
	for (int i = 0; i < cnt; i++) {		// cnt 갯수만큼 Queue를 만들고 각큐에 해당 4자리수를 한자리씩 enqueue실행
